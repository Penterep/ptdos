"""ptdos extension that adds a HTTP HEAD Flood attack."""
...
class HttpHeadFlood:
    ...
    def launch_attack(self, args, dst, duration, use_json, json_obj, json_no, monitoring, att_start_t_epoch, att_start_t_asc) -> None:
        ...
        request = create_request("HEAD", url.hostname, url.query, url.path)
    ...
    def make_help():
        ...
        return [
            {"HTTP HEAD Flood attack options": [
                ["-a", "--attack", "<attack>", "Attack name - httpheadflood"],
...
def register() -> None:
    ...
    factory.register("httpheadflood", HttpHeadFlood)
